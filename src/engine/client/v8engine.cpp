...
CV8Engine::CV8Engine(IGameClient *pClient)
{
	...
	thread_init(ThreadFunc, this);
}

void CV8Engine::ThreadFunc(void *pUser)
{
	CV8Engine *pThis = (CV8Engine *)pUser;
	...
	v8::Locker(pThis->m_pIsolate);
	{
		...
		pThis->m_pIsolate->GetCurrentContext()->Global()->Set(Context, v8pp::to_v8(pThis->m_pIsolate, "game"), Game.new_instance());
